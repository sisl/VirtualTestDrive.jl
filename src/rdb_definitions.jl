...
abstract type RDB_PACKAGE_ELEMENT end
...
function Base.read!(io::IO, structure::RDB_POINT_t)
    structure.x = read(io, Cdouble)
    structure.y = read(io, Cdouble)
    structure.z = read(io, Cdouble)
    structure.flags = read(io, UInt8)
    structure.thetype = read(io, UInt8)
    structure.system = read(io, UInt16)
    structure
end
function Base.write(io::IO, structure::RDB_POINT_t)
    write(io, structure.x)
    write(io, structure.y)
    write(io, structure.z)
    write(io, structure.flags)
    write(io, structure.thetype)
    write(io, structure.system)
end
function Base.show(io::IO, structure::RDB_POINT_t)
    println(io, "RDB_POINT_t:")
    @printf(io, "   x:       %+.16e\n",structure.x)
    @printf(io, "   y:       %+.16e\n",structure.y)
    @printf(io, "   z:       %+.16e\n",structure.z)
    @printf(io, "   flags:   %s\n", hex(structure.flags))
    @printf(io, "   thetype: %s\n", hex(structure.thetype))
    @printf(io, "   system:  %s\n", hex(structure.system))
end
...
function Base.read!(io::IO, structure::RDB_COORD_t)
    structure.x = read(io, Cdouble)
    structure.y = read(io, Cdouble)
    structure.z = read(io, Cdouble)
    structure.h = read(io, Cfloat)
    structure.p = read(io, Cfloat)
    structure.r = read(io, Cfloat)
    structure.flags = read(io, UInt8)
    structure.thetype = read(io, UInt8)
    structure.system = read(io, UInt16)
    structure
end
function Base.write(io::IO, structure::RDB_COORD_t)
    write(io, structure.x)
    write(io, structure.y)
    write(io, structure.z)
    write(io, structure.h)
    write(io, structure.p)
    write(io, structure.r)
    write(io, structure.flags)
    write(io, structure.thetype)
    write(io, structure.system)
end
function Base.show(io::IO, structure::RDB_COORD_t)
...
function Base.read!(io::IO, structure::RDB_COORD_SYSTEM_t)
...
function Base.write(io::IO, structure::RDB_COORD_SYSTEM_t)
    ...
    write(io, structure.spare)
    write(io, structure.pos)
end
function Base.show(io::IO, structure::RDB_COORD_SYSTEM_t)
...
function Base.read!(io::IO, structure::RDB_ROAD_POS_t)
    structure.playerId = read(io, UInt32)
    structure.roadId = read(io, UInt16)
    structure.laneId = read(io, Int8)
    structure.flags = read(io, UInt8)
    structure.roadS = read(io, Cfloat)
    structure.roadT = read(io, Cfloat)
    structure.laneOffset = read(io, Cfloat)
    structure.hdgRel = read(io, Cfloat)
    structure.pitchRel = read(io, Cfloat)
    structure.rollRel = read(io, Cfloat)
    structure.roadType = read(io, UInt8)
    structure.spare1 = read(io, UInt8)
    structure.spare2 = read(io, UInt16)
    structure.pathS = read(io, Cfloat)
    structure
end
function Base.write(io::IO, structure::RDB_ROAD_POS_t)
    write(io, structure.playerId)
    write(io, structure.roadId)
    write(io, structure.laneId)
    write(io, structure.flags)
    write(io, structure.roadS)
    write(io, structure.roadT)
    write(io, structure.laneOffset)
    write(io, structure.hdgRel)
    write(io, structure.pitchRel)
    write(io, structure.rollRel)
    write(io, structure.roadType)
    write(io, structure.spare1)
    write(io, structure.spare2)
    write(io, structure.pathS)
end
function Base.show(io::IO, structure::RDB_ROAD_POS_t)
    println(io, "RDB_ROAD_POS_t:")
    @printf(io, "   playerId:   %s\n", hex(structure.playerId))
    @printf(io, "   roadId:     %s\n", hex(structure.roadId))
    @printf(io, "   laneId:     %d\n",structure.laneId)
    @printf(io, "   flags:      %s\n", hex(structure.flags))
    @printf(io, "   roadS:      %+.16e\n",structure.roadS)
    @printf(io, "   roadT:      %+.16e\n",structure.roadT)
    @printf(io, "   laneOffset: %+.16e\n",structure.laneOffset)
    @printf(io, "   hdgRel:     %+.16e\n",structure.hdgRel)
    @printf(io, "   pitchRel:   %+.16e\n",structure.pitchRel)
    @printf(io, "   rollRel:    %+.16e\n",structure.rollRel)
    @printf(io, "   roadType:   %s\n", hex(structure.roadType))
    @printf(io, "   spare1:     %s\n", hex(structure.spare1))
    @printf(io, "   spare2:     %s\n", hex(structure.spare2))
    @printf(io, "   pathS:      %+.16e\n",structure.pathS)
end
...
function Base.read!(io::IO, structure::RDB_ROADMARK_t)
    structure.playerId = read(io, UInt32)
    structure.id = read(io, Int8)
    structure.prevId = read(io, Int8)
    structure.nextId = read(io, Int8)
    structure.spare0 = read(io, UInt8)
    structure.lateralDist = read(io, Cfloat)
    structure.yawRel = read(io, Cfloat)
    structure.curvHor = read(io, Cdouble)
    structure.curvHorDot = read(io, Cdouble)
    structure.startDx = read(io, Cfloat)
    structure.previewDx = read(io, Cfloat)
    structure.width = read(io, Cfloat)
    structure.height = read(io, Cfloat)
    structure.curvVert = read(io, Cdouble)
    structure.curvVertDot = read(io, Cdouble)
    structure.thetype = read(io, UInt8)
    structure.color = read(io, UInt8)
    structure.noDataPoints = read(io, UInt16)
    structure.spare1 = (read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_ROADMARK_t)
    write(io, structure.playerId)
    write(io, structure.id)
    write(io, structure.prevId)
    write(io, structure.nextId)
    write(io, structure.spare0)
    write(io, structure.lateralDist)
    write(io, structure.yawRel)
    write(io, structure.curvHor)
    write(io, structure.curvHorDot)
    write(io, structure.startDx)
    write(io, structure.previewDx)
    write(io, structure.width)
    write(io, structure.height)
    write(io, structure.curvVert)
    write(io, structure.curvVertDot)
    write(io, structure.thetype)
    write(io, structure.color)
    write(io, structure.noDataPoints)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_ROADMARK_t)
    println(io, "RDB_ROADMARK_t:")
    @printf(io, "   playerId:     %s\n", hex(structure.playerId))
    @printf(io, "   id:           %d\n",structure.id)
    @printf(io, "   prevId:       %d\n",structure.prevId)
    @printf(io, "   nextId:       %d\n",structure.nextId)
    @printf(io, "   spare0:       %s\n", hex(structure.spare0))
    @printf(io, "   lateralDist:  %+.16e\n",structure.lateralDist)
    @printf(io, "   yawRel:       %+.16e\n",structure.yawRel)
    @printf(io, "   curvHor:      %+.16e\n",structure.curvHor)
    @printf(io, "   curvHorDot:   %+.16e\n",structure.curvHorDot)
    @printf(io, "   startDx:      %+.16e\n",structure.startDx)
    @printf(io, "   previewDx:    %+.16e\n",structure.previewDx)
    @printf(io, "   width:        %+.16e\n",structure.width)
    @printf(io, "   height:       %+.16e\n",structure.height)
    @printf(io, "   curvVert:     %+.16e\n",structure.curvVert)
    @printf(io, "   curvVertDot:  %+.16e\n",structure.curvVertDot)
    @printf(io, "   thetype:      %s\n", hex(structure.thetype))
    @printf(io, "   color:        %s\n", hex(structure.color))
    @printf(io, "   noDataPoints: %s\n", hex(structure.noDataPoints))
    @printf(io, "   spare1:       %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_LANE_INFO_t)
    structure.roadId = read(io, UInt16)
    structure.id = read(io, Int8)
    structure.neighborMask = read(io, UInt8)
    structure.leftLaneId = read(io, Int8)
    structure.rightLaneId = read(io, Int8)
    structure.borderType = read(io, UInt8)
    structure.material = read(io, UInt8)
    structure.status = read(io, UInt16)
    structure.thetype = read(io, UInt16)
    structure.width = read(io, Cfloat)
    structure.curvVert = read(io, Cdouble)
    structure.curvVertDot = read(io, Cdouble)
    structure.curvHor = read(io, Cdouble)
    structure.curvHorDot = read(io, Cdouble)
    structure.playerId = read(io, UInt32)
    structure.spare1 = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_LANE_INFO_t)
    write(io, structure.roadId)
    write(io, structure.id)
    write(io, structure.neighborMask)
    write(io, structure.leftLaneId)
    write(io, structure.rightLaneId)
    write(io, structure.borderType)
    write(io, structure.material)
    write(io, structure.status)
    write(io, structure.thetype)
    write(io, structure.width)
    write(io, structure.curvVert)
    write(io, structure.curvVertDot)
...
function Base.show(io::IO, structure::RDB_LANE_INFO_t)
    println(io, "RDB_LANE_INFO_t:")
    @printf(io, "   roadId:       %s\n", hex(structure.roadId))
    @printf(io, "   id:           %d\n",structure.id)
    @printf(io, "   neighborMask: %s\n", hex(structure.neighborMask))
    @printf(io, "   leftLaneId:   %d\n",structure.leftLaneId)
    @printf(io, "   rightLaneId:  %d\n",structure.rightLaneId)
    @printf(io, "   borderType:   %s\n", hex(structure.borderType))
    @printf(io, "   material:     %s\n", hex(structure.material))
    @printf(io, "   status:       %s\n", hex(structure.status))
    @printf(io, "   thetype:      %s\n", hex(structure.thetype))
    @printf(io, "   width:        %+.16e\n",structure.width)
    @printf(io, "   curvVert:     %+.16e\n",structure.curvVert)
    @printf(io, "   curvVertDot:  %+.16e\n",structure.curvVertDot)
...
function Base.read!(io::IO, structure::RDB_OBJECT_CFG_t)
    structure.id = read(io, UInt32)
    ...
    structure.flags = read(io, UInt16)
    structure.spare0 = read(io, UInt16)
    structure.spare1 = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_OBJECT_CFG_t)
    write(io, structure.id)
    write(io, structure.category)
    write(io, structure.thetype)
    write(io, structure.modelId)
    write(io, structure.flags)
    write(io, structure.spare0)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_OBJECT_CFG_t)
    println(io, "RDB_OBJECT_CFG_t:")
    @printf(io, "   id:       %s\n", hex(structure.id))
    @printf(io, "   category: %s\n", hex(structure.category))
    @printf(io, "   thetype:  %s\n", hex(structure.thetype))
    @printf(io, "   modelId:  %d\n",structure.modelId)
    println(io,"   name: ",convert(Array{Char},structure.name))
    println(io,"   modelName: ",convert(Array{Char},structure.modelName))
    println(io,"   fileName: ",convert(Array{Char},structure.fileName))
    @printf(io, "   flags:    %s\n", hex(structure.flags))
    @printf(io, "   spare0:   %s\n", hex(structure.spare0))
    @printf(io, "   spare1:   %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_GEOMETRY_t)
    structure.dimX = read(io, Cfloat)
    structure.dimY = read(io, Cfloat)
    structure.dimZ = read(io, Cfloat)
    structure.offX = read(io, Cfloat)
    structure.offY = read(io, Cfloat)
    structure.offZ = read(io, Cfloat)
    structure
end
function Base.write(io::IO, structure::RDB_GEOMETRY_t)
    write(io, structure.dimX)
    write(io, structure.dimY)
    write(io, structure.dimZ)
    write(io, structure.offX)
    write(io, structure.offY)
    write(io, structure.offZ)
end
function Base.show(io::IO, structure::RDB_GEOMETRY_t)
...
function Base.read!(io::IO, structure::RDB_OBJECT_STATE_BASE_t)
    structure.id = read(io, UInt32)
    structure.category = read(io, UInt8)
    structure.thetype = read(io, UInt8)
    structure.visMask = read(io, UInt16)
    structure.name = (read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar),read(io,Cchar))
    structure.geo = read(io, RDB_GEOMETRY_t)
    structure.pos = read(io, RDB_COORD_t)
    structure.parent = read(io, UInt32)
    structure.spare1 = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_OBJECT_STATE_BASE_t)
    write(io, structure.id)
    write(io, structure.category)
    write(io, structure.thetype)
    write(io, structure.visMask)
    write(io, structure.name)
    write(io, structure.geo)
    write(io, structure.pos)
    write(io, structure.parent)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_OBJECT_STATE_BASE_t)
    println(io, "RDB_OBJECT_STATE_BASE_t:")
    @printf(io, "   id:       %s\n", hex(structure.id))
    @printf(io, "   category: %s\n", hex(structure.category))
    @printf(io, "   thetype:  %s\n", hex(structure.thetype))
    @printf(io, "   visMask:  %s\n", hex(structure.visMask))
    println(io, "name: ",convert(Tuple{Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char,Char},structure.name))
    @printf(io, "   geo:      RDB_GEOMETRY_t\n")
    @printf(io, "   pos:      RDB_COORD_t\n")
    @printf(io, "   parent:   %s\n", hex(structure.parent))
    @printf(io, "   spare1:   %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_OBJECT_STATE_EXT_t)
    structure.speed = read(io, RDB_COORD_t)
...
function Base.write(io::IO, structure::RDB_OBJECT_STATE_EXT_t)
    ...
    write(io, structure.accel)
    write(io, structure.traveledDist)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_OBJECT_STATE_EXT_t)
    println(io, "RDB_OBJECT_STATE_EXT_t:")
    @printf(io, "   speed: RDB_COORD_t\n")
    @printf(io, "   accel: RDB_COORD_t\n")
    @printf(io, "   traveledDist: %f\n",structure.traveledDist)
    @printf(io, "   spare: %s\n", hex(structure.spare))
end
...
function Base.read!(io::IO, structure::RDB_OBJECT_STATE_t)
    structure.base = read(io, RDB_OBJECT_STATE_BASE_t)
    structure.ext = read(io, RDB_OBJECT_STATE_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_OBJECT_STATE_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_OBJECT_STATE_t)
...
function Base.read!(io::IO, structure::RDB_ENGINE_BASE_t)
    structure.playerId = read(io, UInt32)
    structure.rps = read(io, Cfloat)
    structure.load = read(io, Cfloat)
    structure.spare1 = (read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_ENGINE_BASE_t)
    write(io, structure.playerId)
    write(io, structure.rps)
    write(io, structure.load)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_ENGINE_BASE_t)
...
function Base.read!(io::IO, structure::RDB_ENGINE_EXT_t)
    structure.rpsStart = read(io, Cfloat)
    structure.torque = read(io, Cfloat)
    structure.torqueInner = read(io, Cfloat)
    structure.torqueMax = read(io, Cfloat)
    structure.torqueFriction = read(io, Cfloat)
    structure.fuelCurrent = read(io, Cfloat)
    structure.fuelAverage = read(io, Cfloat)
    structure.oilTemperature = read(io, Cfloat)
    structure.temperature = read(io,Cfloat)
    structure
end
function Base.write(io::IO, structure::RDB_ENGINE_EXT_t)
    write(io, structure.rpsStart)
    write(io, structure.torque)
    write(io, structure.torqueInner)
    write(io, structure.torqueMax)
    write(io, structure.torqueFriction)
    write(io, structure.fuelCurrent)
    write(io, structure.fuelAverage)
    write(io, structure.oilTemperature)
    write(io, structure.temperature)
end
function Base.show(io::IO, structure::RDB_ENGINE_EXT_t)
    println(io, "RDB_ENGINE_EXT_t:")
    @printf(io, "   rpsStart:       %+.16e\n",structure.rpsStart)
    @printf(io, "   torque:         %+.16e\n",structure.torque)
    @printf(io, "   torqueInner:    %+.16e\n",structure.torqueInner)
    @printf(io, "   torqueMax:      %+.16e\n",structure.torqueMax)
    @printf(io, "   torqueFriction: %+.16e\n",structure.torqueFriction)
    @printf(io, "   fuelCurrent:    %+.16e\n",structure.fuelCurrent)
    @printf(io, "   fuelAverage:    %+.16e\n",structure.fuelAverage)
    @printf(io, "   oilTemperature:    %+.16e\n",structure.oilTemperature)
...
function Base.read!(io::IO, structure::RDB_ENGINE_t)
    structure.base = read(io, RDB_ENGINE_BASE_t)
    structure.ext = read(io, RDB_ENGINE_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_ENGINE_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_ENGINE_t)
...
function Base.read!(io::IO, structure::RDB_DRIVETRAIN_BASE_t)
    structure.playerId = read(io, UInt32)
    structure.gearBoxType = read(io, UInt8)
    structure.driveTrainType = read(io, UInt8)
    structure.gear = read(io, UInt8)
    structure.spare0 = read(io, UInt8)
    structure.spare1 = (read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_DRIVETRAIN_BASE_t)
    write(io, structure.playerId)
    write(io, structure.gearBoxType)
    write(io, structure.driveTrainType)
    write(io, structure.gear)
    write(io, structure.spare0)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_DRIVETRAIN_BASE_t)
    println(io, "RDB_DRIVETRAIN_BASE_t:")
    @printf(io, "   playerId:       %s\n", hex(structure.playerId))
    @printf(io, "   gearBoxType:    %s\n", hex(structure.gearBoxType))
    @printf(io, "   driveTrainType: %s\n", hex(structure.driveTrainType))
    @printf(io, "   gear:           %s\n", hex(structure.gear))
    @printf(io, "   spare0:         %s\n", hex(structure.spare0))
    @printf(io, "   spare1:         %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_DRIVETRAIN_EXT_t)
    structure.torqueGearBoxIn = read(io, Cfloat)
    structure.torqueCenterDiffOut = read(io, Cfloat)
    structure.torqueShaft = read(io, Cfloat)
    structure.spare1 = (read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_DRIVETRAIN_EXT_t)
    write(io, structure.torqueGearBoxIn)
    write(io, structure.torqueCenterDiffOut)
    write(io, structure.torqueShaft)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_DRIVETRAIN_EXT_t)
    println(io, "RDB_DRIVETRAIN_EXT_t:")
    @printf(io, "   torqueGearBoxIn:     %+.16e\n",structure.torqueGearBoxIn)
    @printf(io, "   torqueCenterDiffOut: %+.16e\n",structure.torqueCenterDiffOut)
    @printf(io, "   torqueShaft:         %+.16e\n",structure.torqueShaft)
    @printf(io, "   spare1:              %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_DRIVETRAIN_t)
    structure.base = read(io, RDB_DRIVETRAIN_BASE_t)
    structure.ext = read(io, RDB_DRIVETRAIN_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_DRIVETRAIN_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_DRIVETRAIN_t)
...
function Base.read!(io::IO, structure::RDB_WHEEL_BASE_t)
    structure.playerId = read(io, UInt32)
    structure.id = read(io, UInt8)
    structure.flags = read(io, UInt8)
    structure.spare0 = (read(io, UInt8), read(io, UInt8))
    structure.radiusStatic = read(io, Cfloat)
    structure.springCompression = read(io, Cfloat)
    structure.rotAngle = read(io, Cfloat)
    structure.slip = read(io, Cfloat)
    structure.steeringAngle = read(io, Cfloat)
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_WHEEL_BASE_t)
    write(io, structure.playerId)
    write(io, structure.id)
    write(io, structure.flags)
    write(io, structure.spare0)
    write(io, structure.radiusStatic)
    write(io, structure.springCompression)
    write(io, structure.rotAngle)
    write(io, structure.slip)
    write(io, structure.steeringAngle)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_WHEEL_BASE_t)
    println(io, "RDB_WHEEL_BASE_t:")
    @printf(io, "   playerId:          %s\n", hex(structure.playerId))
    @printf(io, "   id:                %s\n", hex(structure.id))
    @printf(io, "   flags:             %s\n", hex(structure.flags))
    @printf(io, "   spare0:            %s\n", hex(structure.spare0))
    @printf(io, "   radiusStatic:      %+.16e\n",structure.radiusStatic)
    @printf(io, "   springCompression: %+.16e\n",structure.springCompression)
    @printf(io, "   rotAngle:          %+.16e\n",structure.rotAngle)
    @printf(io, "   slip:              %+.16e\n",structure.slip)
    @printf(io, "   steeringAngle:     %+.16e\n",structure.steeringAngle)
    @printf(io, "   spare1:            %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_WHEEL_EXT_t)
    structure.vAngular = read(io, Cfloat)
    structure.forceZ = read(io, Cfloat)
    structure.forceLat = read(io, Cfloat)
    structure.forceLong = read(io, Cfloat)
    structure.forceTireWheelXYZ = (read(io, Cfloat), read(io, Cfloat), read(io, Cfloat))
    structure.radiusDynamic = read(io, Cfloat)
    structure.brakePressure = read(io, Cfloat)
    structure.torqueDriveShaft = read(io, Cfloat)
    structure.damperSpeed = read(io, Cfloat)
    structure.spare2 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_WHEEL_EXT_t)
    write(io, structure.vAngular)
    write(io, structure.forceZ)
    write(io, structure.forceLat)
    write(io, structure.forceLong)
    write(io, structure.forceTireWheelXYZ)
    write(io, structure.radiusDynamic)
    write(io, structure.brakePressure)
    write(io, structure.torqueDriveShaft)
    write(io, structure.damperSpeed)
    write(io, structure.spare2)
end
function Base.show(io::IO, structure::RDB_WHEEL_EXT_t)
    println(io, "RDB_WHEEL_EXT_t:")
    @printf(io, "   vAngular:          %+.16e\n",structure.vAngular)
    @printf(io, "   forceZ:            %+.16e\n",structure.forceZ)
    @printf(io, "   forceLat:          %+.16e\n",structure.forceLat)
    @printf(io, "   forceLong:         %+.16e\n",structure.forceLong)
    @printf(io, "   forceTireWheelXYZ: %s\n", hex(structure.forceTireWheelXYZ))
    @printf(io, "   radiusDynamic:     %+.16e\n",structure.radiusDynamic)
    @printf(io, "   brakePressure:     %+.16e\n",structure.brakePressure)
    @printf(io, "   torqueDriveShaft:  %+.16e\n",structure.torqueDriveShaft)
    @printf(io, "   damperSpeed:       %+.16e\n",structure.damperSpeed)
    @printf(io, "   spare2:            %s\n", hex(structure.spare2))
end
...
function Base.read!(io::IO, structure::RDB_WHEEL_t)
    structure.base = read(io, RDB_WHEEL_BASE_t)
    structure.ext = read(io, RDB_WHEEL_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_WHEEL_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_WHEEL_t)
...
function Base.read!(io::IO, structure::RDB_VEHICLE_SYSTEMS_t)
    structure.playerId = read(io, UInt32)
    structure.lightMask = read(io, UInt32)
    structure.steering = read(io, Cfloat)
    structure.steeringWheelTorque = read(io, Cfloat)
    structure.accMask = read(io, UInt8)
    structure.accSpeed = read(io, UInt8)
    structure.batteryState = read(io, UInt8)
    structure.batteryRate = read(io, Int8)
    structure.displayLightMask = read(io, UInt16)
    structure.spare0 = read(io, UInt16)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_VEHICLE_SYSTEMS_t)
    write(io, structure.playerId)
    write(io, structure.lightMask)
    write(io, structure.steering)
    write(io, structure.steeringWheelTorque)
    write(io, structure.accMask)
    write(io, structure.accSpeed)
    write(io, structure.batteryState)
    write(io, structure.batteryRate)
    write(io, structure.displayLightMask)
    write(io, structure.spare0)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_VEHICLE_SYSTEMS_t)
    println(io, "RDB_VEHICLE_SYSTEMS_t:")
    @printf(io, "   playerId:            %s\n", hex(structure.playerId))
    @printf(io, "   lightMask:           %s\n", hex(structure.lightMask))
    @printf(io, "   steering:            %+.16e\n",structure.steering)
    @printf(io, "   steeringWheelTorque: %+.16e\n",structure.steeringWheelTorque)
    @printf(io, "   accMask:             %s\n", hex(structure.accMask))
    @printf(io, "   accSpeed:            %s\n", hex(structure.accSpeed))
    @printf(io, "   batteryState:        %s\n", hex(structure.batteryState))
    @printf(io, "   batteryRate:         %d\n",structure.batteryRate)
    @printf(io, "   displayLightMask:    %s\n", hex(structure.displayLightMask))
    @printf(io, "   spare0:              %s\n", hex(structure.spare0))
    @printf(io, "   spare:               %s\n", hex(structure.spare))
end
...
function Base.read!(io::IO, structure::RDB_VEHICLE_SETUP_t)
    structure.playerId = read(io, UInt32)
    structure.mass = read(io, Cfloat)
    structure.wheelBase = read(io, Cfloat)
    structure.spare = (read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32))
    structure
end
function Base.write(io::IO, structure::RDB_VEHICLE_SETUP_t)
    write(io, structure.playerId)
    write(io, structure.mass)
    write(io, structure.wheelBase)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_VEHICLE_SETUP_t)
...
function Base.read!(io::IO, structure::RDB_IMAGE_t)
    structure.id = read(io, UInt32)
    structure.width = read(io, UInt16)
    structure.height = read(io, UInt16)
    structure.pixelSize = read(io, UInt8)
    structure.pixelFormat = read(io, UInt8)
    structure.cameraId = read(io, UInt16)
    structure.imgSize = read(io, UInt32)
    structure.color = (read(io, UInt8), read(io, UInt8), read(io, UInt8), read(io, UInt8))
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_IMAGE_t)
    write(io, structure.id)
    write(io, structure.width)
    write(io, structure.height)
    write(io, structure.pixelSize)
    write(io, structure.pixelFormat)
    write(io, structure.cameraId)
    write(io, structure.imgSize)
    write(io, structure.color)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_IMAGE_t)
    println(io, "RDB_IMAGE_t:")
    @printf(io, "   id:          %s\n", hex(structure.id))
    @printf(io, "   width:       %s\n", hex(structure.width))
    @printf(io, "   height:      %s\n", hex(structure.height))
    @printf(io, "   pixelSize:   %s\n", hex(structure.pixelSize))
    @printf(io, "   pixelFormat: %s\n", hex(structure.pixelFormat))
    @printf(io, "   cameraId:    %s\n", hex(structure.cameraId))
    @printf(io, "   imgSize:     %s\n", hex(structure.imgSize))
    @printf(io, "   color:       %s\n", hex(structure.color))
    @printf(io, "   spare1:      %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_FUNCTION_t)
    structure.id = read(io, UInt32)
    structure.thetype = read(io, UInt8)
    structure.dimension = read(io, UInt8)
    structure.spare = read(io, UInt16)
    structure.dataSize = read(io, UInt32)
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_FUNCTION_t)
    write(io, structure.id)
    write(io, structure.thetype)
    write(io, structure.dimension)
    write(io, structure.spare)
    write(io, structure.dataSize)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_FUNCTION_t)
    println(io, "RDB_FUNCTION_t:")
    @printf(io, "   id:        %s\n", hex(structure.id))
    @printf(io, "   thetype:   %s\n", hex(structure.thetype))
    @printf(io, "   dimension: %s\n", hex(structure.dimension))
    @printf(io, "   spare:     %s\n", hex(structure.spare))
    @printf(io, "   dataSize:  %s\n", hex(structure.dataSize))
    @printf(io, "   spare1:    %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_SENSOR_STATE_t)
    structure.id = read(io, UInt32)
    structure.thetype = read(io, UInt8)
    ...
    structure.fovHV = (read(io, Cfloat), read(io, Cfloat))
    structure.clipNF = (read(io, Cfloat), read(io, Cfloat))
    structure.pos = read(io, RDB_COORD_t)
    structure.originCoordSys = read(io, RDB_COORD_t)
    structure.spare = (read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32))
    structure
end
function Base.write(io::IO, structure::RDB_SENSOR_STATE_t)
    write(io, structure.id)
    write(io, structure.thetype)
    write(io, structure.hostCategory)
    write(io, structure.spare0)
    write(io, structure.hostId)
    for i=1:32
        write(io, structure.name[i])
    end
    write(io, structure.fovHV)
    write(io, structure.clipNF)
    write(io, structure.pos)
    write(io, structure.originCoordSys)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_SENSOR_STATE_t)
    println(io, "RDB_SENSOR_STATE_t:")
    @printf(io, "   id:             %s\n", hex(structure.id))
    @printf(io, "   thetype:        %s\n", hex(structure.thetype))
    @printf(io, "   hostCategory:   %s\n", hex(structure.hostCategory))
    @printf(io, "   spare0:         %s\n", hex(structure.spare0))
    @printf(io, "   hostId:         %s\n", hex(structure.hostId))
    @printf(io, "   fovHV:          %s\n", hex(structure.fovHV))
    @printf(io, "   clipNF:         %s\n", hex(structure.clipNF))
    @printf(io, "   pos:            RDB_COORD_t\n")
...
function Base.read!(io::IO, structure::RDB_SENSOR_OBJECT_t)
    structure.category = read(io, UInt8)
    structure.thetype = read(io, UInt8)
    structure.flags = read(io, UInt16)
    structure.id = read(io, UInt32)
    structure.sensorId = read(io, UInt32)
    structure.dist = read(io, Cdouble)
    structure.sensorPos = read(io, RDB_COORD_t)
    structure.occlusion = read(io, Int8)
    structure.spare0 = (read(io, UInt8), read(io, UInt8), read(io, UInt8))
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_SENSOR_OBJECT_t)
    write(io, structure.category)
    write(io, structure.thetype)
    write(io, structure.flags)
    write(io, structure.id)
    write(io, structure.sensorId)
    write(io, structure.dist)
    write(io, structure.sensorPos)
    write(io, structure.occlusion)
    write(io, structure.spare0)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_SENSOR_OBJECT_t)
    println(io, "RDB_SENSOR_OBJECT_t:")
    @printf(io, "   category:  %s\n", hex(structure.category))
    @printf(io, "   thetype:   %s\n", hex(structure.thetype))
    @printf(io, "   flags:     %s\n", hex(structure.flags))
    @printf(io, "   id:        %s\n", hex(structure.id))
    @printf(io, "   sensorId:  %s\n", hex(structure.sensorId))
    @printf(io, "   dist:      %+.16e\n",structure.dist)
...
function Base.read!(io::IO, structure::RDB_CAMERA_t)
    structure.id = read(io, UInt16)
    structure.width = read(io, UInt16)
    structure.height = read(io, UInt16)
    structure.spare0 = read(io, UInt16)
    structure.clipNear = read(io, Cfloat)
    structure.clipFar = read(io, Cfloat)
    structure.focalX = read(io, Cfloat)
    structure.focalY = read(io, Cfloat)
    structure.principalX = read(io, Cfloat)
    structure.principalY = read(io, Cfloat)
    structure.pos = read(io, RDB_COORD_t)
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_CAMERA_t)
    write(io, structure.id)
    write(io, structure.width)
    write(io, structure.height)
    write(io, structure.spare0)
    write(io, structure.clipNear)
    write(io, structure.clipFar)
    write(io, structure.focalX)
    write(io, structure.focalY)
    write(io, structure.principalX)
    write(io, structure.principalY)
    write(io, structure.pos)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_CAMERA_t)
...
function Base.read!(io::IO, structure::RDB_LIGHT_SOURCE_BASE_t)
    structure.id = read(io, UInt16)
    structure.templateId = read(io, Int8)
    structure.state = read(io, UInt8)
    structure.playerId = read(io, Int32)
    structure.pos = read(io, RDB_COORD_t)
    structure.flags = read(io, UInt16)
    structure.spare0 = read(io, UInt16)
    structure.spare1 = (read(io, Int32), read(io, Int32))
    structure
end
function Base.write(io::IO, structure::RDB_LIGHT_SOURCE_BASE_t)
    write(io, structure.id)
    write(io, structure.templateId)
    write(io, structure.state)
    write(io, structure.playerId)
    write(io, structure.pos)
    write(io, structure.flags)
    write(io, structure.spare0)
...
function Base.show(io::IO, structure::RDB_LIGHT_SOURCE_BASE_t)
...
function Base.read!(io::IO, structure::RDB_LIGHT_SOURCE_EXT_t)
    structure.nearFar = (read(io, Cfloat), read(io, Cfloat))
    structure.frustumLRBT = (read(io, Cfloat), read(io, Cfloat), read(io, Cfloat), read(io, Cfloat))
    structure.intensity = (read(io, Cfloat), read(io, Cfloat), read(io, Cfloat))
    structure.atten = (read(io, Cfloat), read(io, Cfloat), read(io, Cfloat))
    structure.spare1 = (read(io, Int32), read(io, Int32), read(io, Int32))
    structure
end
function Base.write(io::IO, structure::RDB_LIGHT_SOURCE_EXT_t)
    write(io, structure.nearFar)
    write(io, structure.frustumLRBT)
    write(io, structure.intensity)
    write(io, structure.atten)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_LIGHT_SOURCE_EXT_t)
    println(io, "RDB_LIGHT_SOURCE_EXT_t:")
    @printf(io, "   nearFar:     %s\n", hex(structure.nearFar))
    @printf(io, "   frustumLRBT: %s\n", hex(structure.frustumLRBT))
    @printf(io, "   intensity:   %s\n", hex(structure.intensity))
    @printf(io, "   atten:       %s\n", hex(structure.atten))
    @printf(io, "   spare1:      %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_LIGHT_SOURCE_t)
    structure.base = read(io, RDB_LIGHT_SOURCE_BASE_t)
    structure.ext = read(io, RDB_LIGHT_SOURCE_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_LIGHT_SOURCE_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_LIGHT_SOURCE_t)
...
function Base.read!(io::IO, structure::RDB_CONTACT_POINT_t)
    structure.id = read(io, UInt16)
    structure.flags = read(io, UInt16)
    structure.roadDataIn = read(io, RDB_COORD_t)
    structure.friction = read(io, Cfloat)
    structure.playerId = read(io, Int32)
    structure.spare1 = read(io, Int32)
    structure
end
function Base.write(io::IO, structure::RDB_CONTACT_POINT_t)
    write(io, structure.id)
    write(io, structure.flags)
    write(io, structure.roadDataIn)
    write(io, structure.friction)
    write(io, structure.playerId)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_CONTACT_POINT_t)
...
function Base.read!(io::IO, structure::RDB_TRAFFIC_SIGN_t)
    structure.id = read(io, UInt32)
    structure.playerId = read(io, UInt32)
    structure.roadDist = read(io, Cfloat)
    structure.pos = read(io, RDB_COORD_t)
    structure.thetype = read(io, Int32)
    structure.subType = read(io, Int32)
    structure.value = read(io, Cfloat)
    structure.state = read(io, UInt32)
    structure.readability = read(io, Int8)
    structure.occlusion = read(io, Int8)
    structure.spare0 = read(io, UInt16)
    structure.addOnId = read(io, UInt32)
    structure.minLane = read(io, Int8)
    structure.maxLane = read(io, Int8)
    structure.spare = read(io, UInt16)
    structure
end
function Base.write(io::IO, structure::RDB_TRAFFIC_SIGN_t)
    write(io, structure.id)
    write(io, structure.playerId)
    write(io, structure.roadDist)
    write(io, structure.pos)
    write(io, structure.thetype)
    write(io, structure.subType)
    write(io, structure.value)
    write(io, structure.state)
    write(io, structure.readability)
    write(io, structure.occlusion)
    write(io, structure.spare0)
    write(io, structure.addOnId)
    write(io, structure.minLane)
    write(io, structure.maxLane)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_TRAFFIC_SIGN_t)
...
function Base.read!(io::IO, structure::RDB_ROAD_STATE_t)
    structure.playerId = read(io, UInt32)
    structure.wheelId = read(io, Int8)
    structure.spare0 = read(io, UInt8)
    structure.spare1 = read(io, UInt16)
    structure.roadId = read(io, UInt32)
    structure.defaultSpeed = read(io, Cfloat)
    structure.waterLevel = read(io, Cfloat)
    structure.eventMask = read(io, UInt32)
    structure.spare2 = (read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32), read(io, Int32))
    structure
end
function Base.write(io::IO, structure::RDB_ROAD_STATE_t)
    write(io, structure.playerId)
    write(io, structure.wheelId)
    write(io, structure.spare0)
    write(io, structure.spare1)
    write(io, structure.roadId)
    write(io, structure.defaultSpeed)
    write(io, structure.waterLevel)
    write(io, structure.eventMask)
    write(io, structure.spare2)
end
function Base.show(io::IO, structure::RDB_ROAD_STATE_t)
...
function Base.read!(io::IO, structure::RDB_ENVIRONMENT_t)
    structure.visibility = read(io, Cfloat)
    structure.timeOfDay = read(io, UInt32)
    structure.brightness = read(io, Cfloat)
    structure.precipitation = read(io, UInt8)
    structure.cloudState = read(io, UInt8)
    structure.flags = read(io, UInt16)
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_ENVIRONMENT_t)
    write(io, structure.visibility)
    write(io, structure.timeOfDay)
    write(io, structure.brightness)
    write(io, structure.precipitation)
    write(io, structure.cloudState)
    write(io, structure.flags)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_ENVIRONMENT_t)
    println(io, "RDB_ENVIRONMENT_t:")
    @printf(io, "   visibility:    %+.16e\n",structure.visibility)
    @printf(io, "   timeOfDay:     %s\n", hex(structure.timeOfDay))
    @printf(io, "   brightness:    %+.16e\n",structure.brightness)
    @printf(io, "   precipitation: %s\n", hex(structure.precipitation))
    @printf(io, "   cloudState:    %s\n", hex(structure.cloudState))
    @printf(io, "   flags:         %s\n", hex(structure.flags))
    @printf(io, "   spare1:        %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_PED_ANIMATION_t)
    structure.playerId = read(io, UInt32)
    structure.pos = read(io, RDB_COORD_t)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure.noCoords = read(io, UInt32)
    structure.dataSize = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_PED_ANIMATION_t)
    write(io, structure.playerId)
    write(io, structure.pos)
    write(io, structure.spare)
    write(io, structure.noCoords)
    write(io, structure.dataSize)
end
function Base.show(io::IO, structure::RDB_PED_ANIMATION_t)
    println(io, "RDB_PED_ANIMATION_t:")
    @printf(io, "   playerId: %s\n", hex(structure.playerId))
    @printf(io, "   pos:      RDB_COORD_t\n")
    @printf(io, "   spare:    %s\n", hex(structure.spare))
    @printf(io, "   noCoords: %s\n", hex(structure.noCoords))
    @printf(io, "   dataSize: %s\n", hex(structure.dataSize))
end
...
function Base.read!(io::IO, structure::RDB_CUSTOM_SCORING_t)
    structure.playerId = read(io, UInt32)
    structure.pathS = read(io, Cfloat)
    structure.roadS = read(io, Cfloat)
    structure.fuelCurrent = read(io, Cfloat)
    structure.fuelAverage = read(io, Cfloat)
    structure.stateFlags = read(io, UInt32)
    structure.slip = read(io, Cfloat)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_CUSTOM_SCORING_t)
    write(io, structure.playerId)
    write(io, structure.pathS)
    write(io, structure.roadS)
    write(io, structure.fuelCurrent)
    write(io, structure.fuelAverage)
    write(io, structure.stateFlags)
    write(io, structure.slip)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_CUSTOM_SCORING_t)
...
function Base.read!(io::IO, structure::RDB_TRIGGER_t)
    structure.deltaT = read(io, Cfloat)
    structure.frameNo = read(io, UInt32)
    structure.spare = read(io, Int32)
    structure
end
function Base.write(io::IO, structure::RDB_TRIGGER_t)
    write(io, structure.deltaT)
    write(io, structure.frameNo)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_TRIGGER_t)
    println(io, "RDB_TRIGGER_t:")
    @printf(io, "   deltaT:  %+.16e\n",structure.deltaT)
    @printf(io, "   frameNo: %s\n", hex(structure.frameNo))
    @printf(io, "   spare:   %d\n",structure.spare)
end
...
function Base.read!(io::IO, structure::RDB_DRIVER_CTRL_t)
    structure.playerId = read(io, UInt32)
    structure.steeringWheel = read(io, Cfloat)
    structure.steeringSpeed = read(io, Cfloat)
    structure.throttlePedal = read(io, Cfloat)
    structure.brakePedal = read(io, Cfloat)
    structure.clutchPedal = read(io, Cfloat)
    structure.accelTgt = read(io, Cfloat)
    structure.steeringTgt = read(io, Cfloat)
    structure.curvatureTgt = read(io, Cdouble)
    structure.steeringTorque = read(io, Cfloat)
    structure.engineTorqueTgt = read(io, Cfloat)
    structure.speedTgt = read(io, Cfloat)
    structure.gear = read(io, UInt8)
    structure.sourceId = read(io, UInt8)
    structure.spare0 = (read(io, UInt8), read(io, UInt8))
    structure.validityFlags = read(io, UInt32)
    structure.flags = read(io, UInt32)
    structure.mockupInput0 = read(io, UInt32)
    structure.mockupInput1 = read(io, UInt32)
    structure.mockupInput2 = read(io, UInt32)
    structure.spare = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_DRIVER_CTRL_t)
    write(io, structure.playerId)
    write(io, structure.steeringWheel)
    write(io, structure.steeringSpeed)
    write(io, structure.throttlePedal)
    write(io, structure.brakePedal)
    write(io, structure.clutchPedal)
    write(io, structure.accelTgt)
    write(io, structure.steeringTgt)
    write(io, structure.curvatureTgt)
    write(io, structure.steeringTorque)
    write(io, structure.engineTorqueTgt)
    write(io, structure.speedTgt)
    write(io, structure.gear)
    write(io, structure.sourceId)
    write(io, structure.spare0)
    write(io, structure.validityFlags)
    write(io, structure.flags)
    write(io, structure.mockupInput0)
    write(io, structure.mockupInput1)
    write(io, structure.mockupInput2)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_DRIVER_CTRL_t)
    println(io, "RDB_DRIVER_CTRL_t:")
    @printf(io, "   playerId:        %s\n", hex(structure.playerId))
    @printf(io, "   steeringWheel:   %+.16e\n",structure.steeringWheel)
    @printf(io, "   steeringSpeed:   %+.16e\n",structure.steeringSpeed)
    @printf(io, "   throttlePedal:   %+.16e\n",structure.throttlePedal)
    @printf(io, "   brakePedal:      %+.16e\n",structure.brakePedal)
    @printf(io, "   clutchPedal:     %+.16e\n",structure.clutchPedal)
    @printf(io, "   accelTgt:        %+.16e\n",structure.accelTgt)
    @printf(io, "   steeringTgt:     %+.16e\n",structure.steeringTgt)
    @printf(io, "   curvatureTgt:    %+.16e\n",structure.curvatureTgt)
    @printf(io, "   steeringTorque:  %+.16e\n",structure.steeringTorque)
    @printf(io, "   engineTorqueTgt: %+.16e\n",structure.engineTorqueTgt)
    @printf(io, "   speedTgt:        %+.16e\n",structure.speedTgt)
    @printf(io, "   gear:            %s\n", hex(structure.gear))
    @printf(io, "   sourceId:        %s\n", hex(structure.sourceId))
    @printf(io, "   spare0:          %s\n", hex(structure.spare0))
    @printf(io, "   validityFlags:   %s\n", hex(structure.validityFlags))
    @printf(io, "   flags:           %s\n", hex(structure.flags))
    @printf(io, "   mockupInput0:    %s\n", hex(structure.mockupInput0))
    @printf(io, "   mockupInput1:    %s\n", hex(structure.mockupInput1))
    @printf(io, "   mockupInput2:    %s\n", hex(structure.mockupInput2))
    @printf(io, "   spare:           %s\n", hex(structure.spare))
end
...
function Base.read!(io::IO, structure::RDB_DRIVER_PERCEPTION_t)
    structure.playerId = read(io, UInt32)
    structure.speedFromRules = read(io, Cfloat)
    structure.distToSpeed = read(io, Cfloat)
    structure.spare0 = (read(io, Cfloat), read(io, Cfloat), read(io, Cfloat), read(io, Cfloat))
    structure.flags = read(io, UInt32)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_DRIVER_PERCEPTION_t)
    write(io, structure.playerId)
    write(io, structure.speedFromRules)
    write(io, structure.distToSpeed)
    write(io, structure.spare0)
    write(io, structure.flags)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_DRIVER_PERCEPTION_t)
    println(io, "RDB_DRIVER_PERCEPTION_t:")
    @printf(io, "   playerId:       %s\n", hex(structure.playerId))
    @printf(io, "   speedFromRules: %+.16e\n",structure.speedFromRules)
    @printf(io, "   distToSpeed:    %+.16e\n",structure.distToSpeed)
    @printf(io, "   spare0:         %s\n", hex(structure.spare0))
    @printf(io, "   flags:          %s\n", hex(structure.flags))
    @printf(io, "   spare:          %s\n", hex(structure.spare))
end
...
function Base.read!(io::IO, structure::RDB_TRAFFIC_LIGHT_BASE_t)
    structure.id = read(io, Int32)
    structure.state = read(io, Cfloat)
    structure.stateMask = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_TRAFFIC_LIGHT_BASE_t)
    write(io, structure.id)
    write(io, structure.state)
    write(io, structure.stateMask)
end
function Base.show(io::IO, structure::RDB_TRAFFIC_LIGHT_BASE_t)
    println(io, "RDB_TRAFFIC_LIGHT_BASE_t:")
    @printf(io, "   id:        %d\n",structure.id)
    @printf(io, "   state:     %+.16e\n",structure.state)
    @printf(io, "   stateMask: %s\n", hex(structure.stateMask))
end
...
function Base.read!(io::IO, structure::RDB_TRAFFIC_LIGHT_PHASE_t)
    structure.duration = read(io, Cfloat)
    structure.thetype = read(io, UInt8)
    structure.spare = (read(io, UInt8), read(io, UInt8), read(io, UInt8))
    structure
end
function Base.write(io::IO, structure::RDB_TRAFFIC_LIGHT_PHASE_t)
    write(io, structure.duration)
    write(io, structure.thetype)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_TRAFFIC_LIGHT_PHASE_t)
...
function Base.read!(io::IO, structure::RDB_TRAFFIC_LIGHT_EXT_t)
    structure.ctrlId = read(io, Int32)
    structure.cycleTime = read(io, Cfloat)
    structure.noPhases = read(io, UInt16)
    structure.dataSize = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_TRAFFIC_LIGHT_EXT_t)
    write(io, structure.ctrlId)
    write(io, structure.cycleTime)
    write(io, structure.noPhases)
    write(io, structure.dataSize)
end
function Base.show(io::IO, structure::RDB_TRAFFIC_LIGHT_EXT_t)
    println(io, "RDB_TRAFFIC_LIGHT_EXT_t:")
    @printf(io, "   ctrlId:    %d\n",structure.ctrlId)
    @printf(io, "   cycleTime: %+.16e\n",structure.cycleTime)
    @printf(io, "   noPhases:  %s\n", hex(structure.noPhases))
    @printf(io, "   dataSize:  %s\n", hex(structure.dataSize))
end
...
function Base.read!(io::IO, structure::RDB_TRAFFIC_LIGHT_t)
    structure.base = read(io, RDB_TRAFFIC_LIGHT_BASE_t)
    structure.ext = read(io, RDB_TRAFFIC_LIGHT_EXT_t)
    structure
end
function Base.write(io::IO, structure::RDB_TRAFFIC_LIGHT_t)
    write(io, structure.base)
    write(io, structure.ext)
end
function Base.show(io::IO, structure::RDB_TRAFFIC_LIGHT_t)
...
function Base.read!(io::IO, structure::RDB_SYNC_t)
    structure.mask = read(io, UInt32)
    structure.cmdMask = read(io, UInt32)
    structure.systemTime = read(io, Cdouble)
    structure
end
function Base.write(io::IO, structure::RDB_SYNC_t)
    write(io, structure.mask)
    write(io, structure.cmdMask)
    write(io, structure.systemTime)
end
function Base.show(io::IO, structure::RDB_SYNC_t)
    println(io, "RDB_SYNC_t:")
    @printf(io, "   mask:       %s\n", hex(structure.mask))
    @printf(io, "   cmdMask:    %s\n", hex(structure.cmdMask))
...
function Base.read!(io::IO, structure::RDB_ROAD_QUERY_t)
    structure.id = read(io, UInt16)
    structure.flags = read(io, UInt16)
    structure.spare = (read(io, UInt16), read(io, UInt16))
    structure.x = read(io, Cdouble)
    structure.y = read(io, Cdouble)
    structure
end
function Base.write(io::IO, structure::RDB_ROAD_QUERY_t)
    write(io, structure.id)
    write(io, structure.flags)
    write(io, structure.spare)
    write(io, structure.x)
...
function Base.show(io::IO, structure::RDB_ROAD_QUERY_t)
...
function Base.read!(io::IO, structure::RDB_SCP_t)
    structure.version = read(io, UInt16)
    structure.spare = read(io, UInt16)
    structure.dataSize = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_SCP_t)
    write(io, structure.version)
    write(io, structure.spare)
    write(io, structure.dataSize)
end
function Base.show(io::IO, structure::RDB_SCP_t)
    println(io, "RDB_SCP_t:")
    @printf(io, "   version:  %s\n", hex(structure.version))
    @printf(io, "   spare:    %s\n", hex(structure.spare))
    @printf(io, "   dataSize: %s\n", hex(structure.dataSize))
end
...
function Base.read!(io::IO, structure::RDB_PROXY_t)
    structure.protocol = read(io, UInt16)
    structure.pkgId = read(io, UInt16)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure.dataSize = read(io, UInt32)
    structure
end
function Base.write(io::IO, structure::RDB_PROXY_t)
    write(io, structure.protocol)
    write(io, structure.pkgId)
    write(io, structure.spare)
    write(io, structure.dataSize)
end
function Base.show(io::IO, structure::RDB_PROXY_t)
    println(io, "RDB_PROXY_t:")
    @printf(io, "   protocol: %s\n", hex(structure.protocol))
    @printf(io, "   pkgId:    %s\n", hex(structure.pkgId))
    @printf(io, "   spare:    %s\n", hex(structure.spare))
    @printf(io, "   dataSize: %s\n", hex(structure.dataSize))
end
...
function Base.read!(io::IO, structure::RDB_TRAJECTORY_t)
    structure.playerId = read(io, UInt32)
    structure.spacing = read(io, Cdouble)
    structure.flags = read(io, UInt16)
    structure.noDataPoints = read(io, UInt16)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_TRAJECTORY_t)
    write(io, structure.playerId)
    write(io, structure.spacing)
    write(io, structure.flags)
    write(io, structure.noDataPoints)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_TRAJECTORY_t)
...
function Base.read!(io::IO, structure::RDB_MOTION_SYSTEM_t)
    structure.playerId = read(io, UInt32)
    structure.flags = read(io, UInt32)
    structure.pos = read(io, RDB_COORD_t)
    structure.speed = read(io, RDB_COORD_t)
...
function Base.write(io::IO, structure::RDB_MOTION_SYSTEM_t)
    write(io, structure.playerId)
    write(io, structure.flags)
    write(io, structure.pos)
    write(io, structure.speed)
...
function Base.show(io::IO, structure::RDB_MOTION_SYSTEM_t)
...
function Base.show(io::IO, structure::RDB_END_OF_FRAME_t)
...
function Base.read!(io::IO, structure::RDB_START_OF_FRAME_t)
    structure
end
function Base.write(io::IO, structure::RDB_START_OF_FRAME_t)
end
function Base.show(io::IO, structure::RDB_START_OF_FRAME_t)
...
function Base.read!(io::IO, structure::RDB_STEER_2_DYN_t)
    structure.playerId = read(io, UInt32)
    structure.state = read(io, UInt32)
    structure.angle = read(io, Cfloat)
    structure.rev = read(io, Cfloat)
    structure.torque = read(io, Cfloat)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_STEER_2_DYN_t)
    write(io, structure.playerId)
    write(io, structure.state)
    write(io, structure.angle)
    write(io, structure.rev)
    write(io, structure.torque)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_STEER_2_DYN_t)
...
function Base.read!(io::IO, structure::RDB_DYN_2_STEER_t)
    structure.playerId = read(io, UInt32)
    structure.state = read(io, UInt16)
    structure.cmd = read(io, UInt16)
    structure.effects = read(io, UInt32)
    structure.torque = read(io, Cfloat)
    structure.friction = read(io, Cfloat)
    structure.damping = read(io, Cfloat)
    structure.stiffness = read(io, Cfloat)
    structure.velocity = read(io, Cfloat)
    structure.angle = read(io, Cfloat)
    structure.spare = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_DYN_2_STEER_t)
    write(io, structure.playerId)
    write(io, structure.state)
    write(io, structure.cmd)
    write(io, structure.effects)
    write(io, structure.torque)
    write(io, structure.friction)
    write(io, structure.damping)
    write(io, structure.stiffness)
    write(io, structure.velocity)
    write(io, structure.angle)
    write(io, structure.spare)
end
function Base.show(io::IO, structure::RDB_DYN_2_STEER_t)
    println(io, "RDB_DYN_2_STEER_t:")
    @printf(io, "   playerId:  %s\n", hex(structure.playerId))
    @printf(io, "   state:     %s\n", hex(structure.state))
    @printf(io, "   cmd:       %s\n", hex(structure.cmd))
    @printf(io, "   effects:   %s\n", hex(structure.effects))
    @printf(io, "   torque:    %+.16e\n",structure.torque)
    @printf(io, "   friction:  %+.16e\n",structure.friction)
...
# end
# function Base.read!(io::IO, structure::RDB_MSG_HDR_t)
#     structure.magicNo = read(io, UInt16)
#     structure.version = read(io, UInt16)
...
#     struct
# end
# function Base.write(io::IO, structure::RDB_MSG_HDR_t)
#     write(io, structure.magicNo)
#     write(io, structure.version)
#     write(io, structure.headerSize)
#     write(io, structure.dataSize)
#     write(io, structure.frameNo)
#     write(io, structure.simTime)
# end
# function Base.show(io::IO, structure::RDB_MSG_HDR_t)
#     println(io, "RDB_MSG_HDR_t:")
#     @printf(io, "   magicNo:    %s\n", hex(structure.magicNo))
#     @printf(io, "   version:    %s\n", hex(structure.version))
#     @printf(io, "   headerSize: %s\n", hex(structure.headerSize))
#     @printf(io, "   dataSize:   %s\n", hex(structure.dataSize))
#     @printf(io, "   frameNo:    %s\n", hex(structure.frameNo))
...
function Base.read!(io::IO, structure::RDB_MSG_ENTRY_HDR_t)
    structure.headerSize = read(io, UInt32)
    structure.dataSize = read(io, UInt32)
    structure.elementSize = read(io, UInt32)
    structure.pkgId = read(io, UInt16)
    structure.flags = read(io, UInt16)
    structure
end
function Base.write(io::IO, structure::RDB_MSG_ENTRY_HDR_t)
    write(io, structure.headerSize)
    write(io, structure.dataSize)
    write(io, structure.elementSize)
    write(io, structure.pkgId)
    write(io, structure.flags)
end
function Base.show(io::IO, structure::RDB_MSG_ENTRY_HDR_t)
    println(io, "RDB_MSG_ENTRY_HDR_t:")
    @printf(io, "   headerSize:  %s\n", hex(structure.headerSize))
    @printf(io, "   dataSize:    %s\n", hex(structure.dataSize))
    @printf(io, "   elementSize: %s\n", hex(structure.elementSize))
    @printf(io, "   pkgId:       %s\n", hex(structure.pkgId))
    @printf(io, "   flags:       %s\n", hex(structure.flags))
end
...
# end
# function Base.read!(io::IO, structure::RDB_MSG_t)
#     structure.hdr = read(io, RDB_MSG_HDR_t)
#     structure.entryHdr = read(io, RDB_MSG_ENTRY_HDR_t)
#     structure.u = read(io, RDB_MSG_UNION_t)
#     struct
# end
# function Base.write(io::IO, structure::RDB_MSG_t)
#     write(io, structure.hdr)
#     write(io, structure.entryHdr)
#     write(io, structure.u)
# end
# function Base.show(io::IO, structure::RDB_MSG_t)
...
function Base.read!(io::IO, structure::RDB_SHM_BUFFER_INFO_t)
    structure.thisSize = read(io, UInt32)
    structure.bufferSize = read(io, UInt32)
    structure.id = read(io, UInt16)
    structure.spare0 = read(io, UInt16)
    structure.flags = read(io, UInt32)
    structure.offset = read(io, UInt32)
    structure.spare1 = (read(io, UInt32), read(io, UInt32), read(io, UInt32), read(io, UInt32))
    structure
end
function Base.write(io::IO, structure::RDB_SHM_BUFFER_INFO_t)
    write(io, structure.thisSize)
    write(io, structure.bufferSize)
    write(io, structure.id)
    write(io, structure.spare0)
    write(io, structure.flags)
    write(io, structure.offset)
    write(io, structure.spare1)
end
function Base.show(io::IO, structure::RDB_SHM_BUFFER_INFO_t)
    println(io, "RDB_SHM_BUFFER_INFO_t:")
    @printf(io, "   thisSize:   %s\n", hex(structure.thisSize))
    @printf(io, "   bufferSize: %s\n", hex(structure.bufferSize))
    @printf(io, "   id:         %s\n", hex(structure.id))
    @printf(io, "   spare0:     %s\n", hex(structure.spare0))
    @printf(io, "   flags:      %s\n", hex(structure.flags))
    @printf(io, "   offset:     %s\n", hex(structure.offset))
    @printf(io, "   spare1:     %s\n", hex(structure.spare1))
end
...
function Base.read!(io::IO, structure::RDB_SHM_HDR_t)
    structure.headerSize = read(io, UInt32)
    structure.dataSize = read(io, UInt32)
    structure.noBuffers = read(io, UInt8)
    structure
end
function Base.write(io::IO, structure::RDB_SHM_HDR_t)
    write(io, structure.headerSize)
    write(io, structure.dataSize)
    write(io, structure.noBuffers)
end
function Base.show(io::IO, structure::RDB_SHM_HDR_t)
    println(io, "RDB_SHM_HDR_t:")
    @printf(io, "   headerSize: %s\n", hex(structure.headerSize))
    @printf(io, "   dataSize:   %s\n", hex(structure.dataSize))
    @printf(io, "   noBuffers:  %s\n", hex(structure.noBuffers))
end
