# TODO
#  - create a new folder, ViresCSV.jl
#  - create a src folder and a test folder
#  - move visualize_CSV.jl into ViresCSV.jl/src/ and rename it to ViresCSV.jl
#  - create a runtests.jl in ViresCSV.jl/src
#  - take "run code" for this file and put it in runtests.jl
#  - turn ViresCSV.jl into a module! (module, export)
#  - make sure you can run runtests.jl
#  - using Base.Test in runtests.jl and create some test cases
#      - for each function - create test cases for it
#      - create a fake CSV file (short one, 2 frames, 2 cars)
#      - make sure that it can be parsed correctly
#      - read_vires_csv()
#      - @test csv.header[:some_key] = what_it_should_be
...
type PlayerHeader
    ...
    length::Float64                 
    width::Float64        
    center_offset_x::Float64         
    center_offset_y::Float64        
    model::Int
    category::Int        
    ctrl::Int      
    ctrl_arg::Int              
...
function set!(csv::ViresCSV, carind::Int, frameind::Int, value::Any)
    @assert(inbounds(csv, carind, frameind))
    str = get_column_symbol(carind, str)
    csv.body[frameind, str] = value
    csv
end
...
function export_to_bosch_csv(io::IO, csv::ViresCSV;
    start_frame::Int=3, # first frame to export
